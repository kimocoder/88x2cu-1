...
#define PS_CTRL_DATAPATH 0 /* whether datapath involved in ps */
...
static bool _chk_rssi_diff_reach_thld(struct cmd_ps *ps)
{
	...
	do {
		if (*rssi_bcn_min == 0 || *rssi_bcn_min == 0xFF) {
			PHL_INFO("[PS_CMD], %s(): (criteria = %d, cur = %d) criteria invalid, set criteria to cur\n",
		...
		if (DIFF(*rssi_bcn_min, cur_rssi_bcn_min) < ps_cap->lps_rssi_diff_threshold) {
			PHL_INFO("[PS_CMD], %s(): (criteria = %d, cur = %d) RSSI diff < %d, do nothing\n",
		...
		*rssi_bcn_min = cur_rssi_bcn_min;
		PHL_INFO("[PS_CMD], %s(): leave ps and update criteria to %d\n", __func__, cur_rssi_bcn_min);
...
_lps_state_judge_changed(struct cmd_ps *ps, u16 macid, u8 cur_state, u8 *new_state)
{
	...
	*new_state = cur_state;
	...
	if (sta == NULL) {
		PHL_WARN("[PS_CMD], %s(): cannot get sta of macid %d.\n", __func__, macid);
		return false;
	}

	rssi = rtw_hal_get_sta_rssi(sta);

	if (cur_state == PS_STATE_LEAVED) {
	...
	if (*new_state != cur_state) {
		PHL_INFO("[PS_CMD], %s(): state %s -> %s, Tx(%d), Rx(%d), RSSI(%d)\n",
 ...
 * @ps: see cmd_ps
 */
enum rtw_phl_status _leave_ps(struct cmd_ps *ps)
{
	struct phl_info_t *phl_info = ps->phl_info;
	...
	if (ps->cur_pwr_lvl == PS_PWR_LVL_PWRON) {
		PHL_INFO("[PS_CMD], %s(): already power on.\n", __func__);
		return RTW_PHL_STATUS_SUCCESS;
	} else {
		if (ps->ps_mode == PS_MODE_LPS) {
			PHL_INFO("[PS_CMD], %s(): target macid to leave lps %d.\n", __func__, ps->macid);
			cfg.macid = ps->macid;
			cfg.token = &ps->null_token;
			cfg.pwr_lvl = phl_ps_judge_pwr_lvl(ps_cap->lps_cap, ps->ps_mode, false);
			cfg.ps_mode = ps->ps_mode;
		} else if (ps->ps_mode == PS_MODE_IPS) {
			cfg.pwr_lvl = PS_PWR_LVL_PWRON;
			cfg.ps_mode = ps->ps_mode;
		} else {
			PHL_ERR("[PS_CMD], %s(): unknown ps mode!\n", __func__);
		}
		...
		if (status == RTW_PHL_STATUS_SUCCESS) {
			...
			PHL_INFO("[PS_CMD], %s(): leave ps success.\n", __func__);
			PHL_INFO("[PS_CMD], %s(): ps mode(%s), pwr lvl(%s), ps state(%s)\n",
					__func__, phl_ps_ps_mode_to_str(ps->ps_mode),
					phl_ps_pwr_lvl_to_str(ps->cur_pwr_lvl), _ps_state_to_str(ps->ps_state));
		} else {
			PHL_ERR("[PS_CMD], %s(): leave ps fail!\n", __func__);
		}
	}
...
enum rtw_phl_status _enter_ps(struct cmd_ps *ps, u8 ps_mode, u16 macid)
{
	...
	if (ps->ps_state == PS_STATE_ENTERED) {
		PHL_WARN("[PS_CMD], %s(): already in ps state.\n", __func__);
		return RTW_PHL_STATUS_SUCCESS;
	}
	...
	if (ps_mode == PS_MODE_LPS) {
		PHL_INFO("[PS_CMD], %s(): target macid to enter lps %d.\n", __func__, macid);
		cfg.macid = macid;
		cfg.token = &ps->null_token;
	...
	if (status == RTW_PHL_STATUS_SUCCESS) {
		...
		PHL_INFO("[PS_CMD], %s(): enter ps success.\n", __func__);
		PHL_INFO("[PS_CMD], %s(): ps mode(%s), pwr lvl(%s), ps state(%s)\n",
					__func__, phl_ps_ps_mode_to_str(ps->ps_mode),
					phl_ps_pwr_lvl_to_str(ps->cur_pwr_lvl), _ps_state_to_str(ps->ps_state));
	} else {
		PHL_ERR("[PS_CMD], %s(): enter ps fail!\n", __func__);
	}
...
static bool _pop_idle_req(struct cmd_ps *ps, struct pwr_req **req)
{
	...
	PHL_INFO("[PS_CMD], %s(): remaining idle req cnt %d.\n", __func__, ps->req_idle_q.cnt);
...
static bool _pop_busy_req(struct cmd_ps *ps, struct pwr_req **req)
{
	...
	PHL_INFO("[PS_CMD], %s(): current busy req cnt %d.\n", __func__, ps->req_busy_q.cnt);
...
static void _cancel_pwr_req(struct cmd_ps *ps, u16 evt_id)
{
	...
	if (!_pop_busy_req(ps, &req)) {
		PHL_WARN("[PS_CMD], %s(): busy queue is empty.\n", __func__);
		return;
	}

	PHL_INFO("[PS_CMD], %s(): evt_id %d\n", __func__, req->evt_id);

	if (req->evt_id != evt_id && MSG_EVT_PHY_IDLE != evt_id)
		PHL_WARN("[PS_CMD], %s(): evt_id mismatch.\n", __func__);
...
static enum rtw_phl_status _add_pwr_req(struct cmd_ps *ps, u16 evt_id)
{
	...
	if (!_pop_idle_req(ps, &req)) {
		PHL_ERR("[PS_CMD], %s(): idle queue is empty.\n", __func__);
		...
		u8 target_mode, u16 *macid)
{
	...
	if (ps->mr_info.ap_active || ps->mr_info.gc_active) {
		PHL_INFO("[PS_CMD], %s(): ap %d or gc %d, is active.\n", __func__,
	...
	if (target_mode == PS_MODE_IPS) {
		...
		if (role == NULL) {
			PHL_WARN("[PS_CMD], %s(): there is no role to enter ps.\n", __func__);
			return false;
		}
		sta = rtw_phl_get_stainfo_self(ps->phl_info, role);
		if (sta == NULL) {
			PHL_WARN("[PS_CMD], %s(): cannot get sta info.\n", __func__);
			return false;
		}
		PHL_INFO("[PS_CMD], %s(): role id to enter ps (%d).\n", __func__, role->id);
		*macid = sta->macid;
		ret = true;
	}

	PHL_INFO("[PS_CMD], %s(): decide enter ps(%s), target mode(%s).\n",
...
static void _stop_datapath(struct cmd_ps *ps)
{
	...
	if (phl_disp_eng_send_msg(phl_info, &msg, &attr, NULL) !=
		...
		PHL_ERR("[PS_CMD], %s(): fail to send tx pause msg.\n", __func__);
	}
}
...
static bool _chk_ps_cap(struct cmd_ps *ps, u8 mode)
{
	...
	PHL_INFO("[PS_CMD], %s(): mode(%s), lps_en(%d), ips_en(%d), runtime_ps_en(%d).\n",
	...
	case PS_MODE_IPS:
		...
		break;
	default:
		PHL_WARN("[PS_CMD], %s(): unknown ps mode.\n", __func__);
		return false;
	}
...
static bool _chk_ips_enter(struct cmd_ps *ps)
{
	if (!_chk_ps_cap(ps, PS_MODE_IPS)) {
		PHL_INFO("[PS_CMD], %s(): ips is not allowed.\n", __func__);
		return false;
	}

	if (ps->req_busy_q.cnt) {
		PHL_INFO("[PS_CMD], %s(): req q is not empty.\n", __func__);
		return false;
	}

	if (ps->btc_req_pwr) {
		PHL_INFO("[PS_CMD], %s(): btc req pwr.\n", __func__);
		return false;
	}

	if (!_chk_wrole_with_ps_mode(ps, PS_MODE_IPS, NULL)) {
		PHL_INFO("[PS_CMD], %s(): no need to enter ips.\n", __func__);
		return false;
	}
...
static bool _chk_lps_enter(struct cmd_ps *ps, u16 *macid)
{
	...
	if (!_chk_ps_cap(ps, PS_MODE_LPS)) {
		PHL_INFO("[PS_CMD], %s(): lps is not allowed.\n", __func__);
		return false;
	}
	...
	if (ps->pre_phase_leave_ps == true) {
		PHL_INFO("[PS_CMD], %s(): just leave ps in watchdog prephase.\n", __func__);
		return false;
	}

	/* check wifi role */
	if (!_chk_wrole_with_ps_mode(ps, PS_MODE_LPS, macid)) {
		PHL_INFO("[PS_CMD], %s(): no need to enter lps.\n", __func__);
		return false;
	}

	/* lps */
	if (_lps_state_judge_changed(ps, *macid, ps->ps_state, &target_ps_state)) {
		PHL_INFO("[PS_CMD], %s(): lps state changed.\n", __func__);
		if (target_ps_state == PS_STATE_ENTERED) {
			PHL_INFO("[PS_CMD], %s(): try enter lps.\n", __func__);
...
{
	PHL_INFO("========== PS CMD Info ========== \n");
	PHL_INFO("ps mode(%s), pwr lvl(%s), ps state(%s)\n",
			phl_ps_ps_mode_to_str(ps->ps_mode),
			phl_ps_pwr_lvl_to_str(ps->cur_pwr_lvl),
			_ps_state_to_str(ps->ps_state));
	PHL_INFO("req idle cnt(%d), req busy cnt(%d)\n",
...
_ps_watchdog_pre_hdlr(struct cmd_ps *ps)
{
	...
	if (ps->cur_pwr_lvl == PS_PWR_LVL_PWRON) {
		PHL_INFO("[PS_CMD], %s(): already power on.\n", __func__);
		return MDL_RET_SUCCESS;
	} else if (ps->cur_pwr_lvl == PS_PWR_LVL_PWROFF) {
		PHL_INFO("[PS_CMD], %s(): power off.\n", __func__);
		return MDL_RET_CANNOT_IO;
	} else {
		if (_lps_state_judge_changed(ps, ps->macid, ps->ps_state, &target_ps_state)) {
			PHL_INFO("[PS_CMD], %s(): lps state changed.\n", __func__);
			if (target_ps_state == PS_STATE_LEAVED) {
				PHL_INFO("[PS_CMD], %s(): try leave lps.\n", __func__);
				if (_leave_ps(ps) == RTW_PHL_STATUS_SUCCESS) {
					ps->pre_phase_leave_ps = true;
					return MDL_RET_SUCCESS;
				} else {
					PHL_ERR("[PS_CMD], %s(): going to L2 reset!\n", __func__);
					return MDL_RET_CANNOT_IO;
				}
			}
		}
...
static enum phl_mdl_ret_code _phy_idle_msg_hdlr(struct cmd_ps *ps, struct phl_msg *msg)
{
	...
	if (!IS_MSG_IN_PRE_PHASE(msg->msg_id)) {
		...
		PHL_INFO("[PS_CMD], %s(): try enter ips.\n", __func__);
...
static void _ps_mr_info_upt(struct cmd_ps *ps, struct rtw_wifi_role_t *role)
{
	...
	PHL_INFO("[PS_CMD], %s(): id %d, type %d, mstate %d\n", __func__,
				role->id, role->type, role->mstate);
	#ifdef RTW_WKARD_PHL_NTFY_MEDIA_STS
	PHL_INFO("[PS_CMD], %s(): is_gc %d\n", __func__, role->is_gc);
	#endif
	...
	PHL_INFO("[PS_CMD], %s(): gc_active %d, ap_active %d\n", __func__,
...
_mrc_mdl_msg_hdlr(struct cmd_ps *ps, struct phl_msg *msg)
{
	...
	default:
		if (ps->cur_pwr_lvl != PS_PWR_LVL_PWRON) {
			PHL_WARN("[PS_CMD], %s(): MDL_ID(%d)-EVT_ID(%d) get cannot I/O!\n", __func__,
...
_general_mdl_msg_hdlr(struct cmd_ps *ps, struct phl_msg *msg)
{
	...
	case MSG_EVT_WATCHDOG:
		if (IS_MSG_IN_PRE_PHASE(msg->msg_id)) {
			...
			if (ret == MDL_RET_SUCCESS) {
				...
				PHL_INFO("[PS_CMD], %s(): update rssi_bcn_min to %d\n", __func__, ps->rssi_bcn_min);
			}
		} else {
	...
	case MSG_EVT_RF_OFF:
		if (!IS_MSG_IN_PRE_PHASE(msg->msg_id)) {
			PHL_INFO("[PS_CMD], MSG_EVT_RF_OFF\n");
			ps->rej_pwr_req = true;
			_enter_ps(ps, PS_MODE_IPS, 0xFFFF);
		}
		ret = MDL_RET_SUCCESS;
...
{
	if (IS_MSG_IN_PRE_PHASE(msg->msg_id)) {
		PHL_INFO("[PS_CMD], %s(): update runtime cap %d.\n", __func__, (bool)*msg->inbuf);
...
_ps_mdl_set_info(void *dispr, void *priv, struct phl_module_op_info *info)
{
	...
	case PS_MDL_OP_BTC_PWR_REQ:
		PHL_INFO("[PS_CMD], %s(): btc req pwr %d\n", __func__, *(bool *)info->inbuf);
...
_ps_mdl_query_info(void *dispr, void *priv, struct phl_module_op_info *info)
{
	...
	case PS_MDL_OP_CUR_PWR_LVL:
		...
		_os_mem_cpy(phl_to_drvpriv(ps->phl_info), (void *)info->outbuf,
...
bool phl_ps_is_datapath_allowed(struct phl_info_t *phl_info)
{
	...
	op_info.outbuf = (u8 *)&pwr_lvl;
	op_info.outlen = sizeof(pwr_lvl);

	phl_disp_eng_query_bk_module_info(phl_info, HW_BAND_0,
			PHL_MDL_POWER_MGNT, &op_info);

	if (pwr_lvl > PS_PWR_LVL_PWR_GATED)
		io_allowed = true;

	PHL_INFO("[PS_CMD], %s(): pwr lvl(%s), io_allowed(%d).\n",
			__func__, phl_ps_pwr_lvl_to_str(pwr_lvl), io_allowed);
...
void rtw_phl_ps_set_rt_cap(void *phl, u8 band_idx, bool ps_allow)
{
	...
	if (val == NULL) {
		PHL_ERR("[PS_CMD], %s(): fail to alloc memory.\n", __func__);
		return;
	}
	...
	*val = ps_allow;
	...
	if (phl_disp_eng_send_msg(phl_info, &msg, &attr, NULL) !=
				RTW_PHL_STATUS_SUCCESS) {
		PHL_ERR("[PS_CMD], %s(): fail to notify batter change.\n", __func__);
		goto cmd_fail;
	}
