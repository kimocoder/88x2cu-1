...
static inline u32 _os_get_performance_time_us(void)
{
	u64 ret;
	ret = PlatformGetPerformanceTime();
	return (u32)ret;
}
#endif
...
static inline void _os_pkt_buf_free_rx(void *d, u8 *vir_addr, _dma bus_addr_l,
			_dma bus_addr_h, u32 buf_sz, void *os_priv)
{
	struct _SHARED_MEMORY ptr;

	if (NULL != vir_addr) {
		ptr.VirtualAddress = (pu1Byte)vir_addr;
		ptr.PhysicalAddressLow = (u4Byte)bus_addr_l;
		ptr.PhysicalAddressHigh = (u4Byte)bus_addr_h;
		ptr.Length = (u4Byte)buf_sz;

		PlatformFreeSharedMemory(d, &ptr);
	}
}
...
static inline void *_os_shmem_alloc(void *d, _dma *bus_addr_l,
			_dma *bus_addr_h, u32 buf_sz, u8 cache, u8 direction)
{
	struct _SHARED_MEMORY share_mem;
...
{
	PVOID ptr = NULL;
	if (PlatformAllocateMemoryWithTag(h, tag, &ptr, buf_sz) != RT_STATUS_SUCCESS)
		return NULL;
