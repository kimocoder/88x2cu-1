			...
			struct rtw_chlist *filter)
{
	...
	for (i = 0; i < filter->cnt; i++) {
		for (j = 0; j < inplan.cnt; j++) {
			if (filter->ch[i] == inplan.ch[j].channel) {
...
static bool _query_channel(struct rtw_regulation *rg, u16 channel,
				struct rtw_regulation_channel *ch)
{
	struct rtw_regulation_chplan_group *plan = NULL;
	u8 i = 0, j = 0;

	if ((CH_2GHZ(channel) && !(rg->capability & CAPABILITY_2GHZ)) ||
		(CH_5GHZ(channel) && !(rg->capability & CAPABILITY_5GHZ)))
		return false;
			...
			struct rtw_regulation_chplan *plan)
{
	...
	u16 i = 0, j = 0, ch = 0, passive = 0, dfs = 0, cnt = 0;
	u32 shift = 0;
	u8 group = FREQ_GROUP_5GHZ_BAND1;
	u8 max_num = 0, ch_start = 0;
	u8 array_idx = 0;
...
bool rtw_phl_query_regulation_info(void *phl, struct rtw_regulation_info *info)
{
	...
	info->domain_code = rg->domain.code;
...
bool rtw_phl_regulation_valid_channel(void *phl, u16 channel, u8 reject)
{
	...
	_os_spinlock(d, &rg->lock, _bh, NULL);
	if (_query_channel(rg, channel, &ch)) {
