...
void rtw_hal_rf_reg_dump(void *sel, void *h)
{
	...
	u32 value;
	u8 path_nums = hal_com->rf_path_num;
	...
	for (path = 0; path < path_nums; path++) {
		...
		for (i = 0; i < 0x100; i++) {
			value = rtw_hal_read_rfreg(hal, path, i, 0xffffffff);
			if (j % 4 == 1)
...
static enum rtw_hal_status hal_ops_check(struct hal_info_t *hal)
{
	...
	if (!trx_ops->cfg_dma_io) {
		hal_error_msg("trx cfg_dma_io");
		status = RTW_HAL_STATUS_FAILURE;
	}
	if (!trx_ops->cfg_txdma) {
		hal_error_msg("trx cfg_txdma");
		status = RTW_HAL_STATUS_FAILURE;
	}
	...
	if (!trx_ops->cfg_txhci) {
		hal_error_msg("trx cfg_txhci");
		status = RTW_HAL_STATUS_FAILURE;
	}
	if (!trx_ops->cfg_rxhci) {
		hal_error_msg("trx cfg_rxhci");
		status = RTW_HAL_STATUS_FAILURE;
	}
	if (!trx_ops->clr_rwptr) {
		hal_error_msg("trx clr_rwptr");
		status = RTW_HAL_STATUS_FAILURE;
	}
	if (!trx_ops->rst_bdram) {
		hal_error_msg("trx rst_bdram");
		status = RTW_HAL_STATUS_FAILURE;
	}
	...
	if (!trx_ops->cfg_rsvd_ctrl) {
		hal_error_msg("trx cfg_rsvd_ctrl");
		status = RTW_HAL_STATUS_FAILURE;
	}
...
enum rtw_hal_status rtw_hal_get_pwr_state(void *hal, u8 *pwr_state)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
