...
u32 sec_info_tbl_init(struct mac_adapter *adapter)
{
	u8 i = 0;
	...
	if ((*sec_cam_table) == NULL) {
		...
		for (i = 0; i < SEC_CAM_ENTRY_NUM; i++) {
			...
			(struct mac_sec_cam_info *)PLTFM_MALLOC
			(sizeof(struct mac_sec_cam_info));
...
u32 free_sec_info_tbl(struct mac_adapter *adapter)
{
	u8 i;
...
u32 fill_sec_cam_info(struct mac_adapter *adapter,
...
u32 fill_addr_cam_sec_only(struct mac_adapter *adapter,
...
u32 mac_upd_sec_infotbl(struct mac_adapter *adapter,
...
fail:
		...
		/* Indirect Access */
		s_info = (struct mac_sec_cam_info *)info;
...
u8 decide_sec_cam_index(struct mac_adapter *adapter, u8 *sec_cam_idx)
{
	u8 sec_idx = 0, i = 0;
	/* call by pointer */
	struct sec_cam_table_t **sec_cam_table =
...
u8 delete_key_from_addr_cam(struct mac_adapter *adapter,
...
u8 insert_key_to_addr_cam(struct mac_adapter *adapter,
			  struct mac_role_tbl *role, u8 key_type,
			  u8 key_id, u8 sec_cam_idx)
{
	u32 ret = 0;
...
u32 m_security_cam_hal(struct mac_adapter *adapter,
		       struct mac_sec_cam_info *sec_cam_info,
...
u32 disconnect_flush_key(struct mac_adapter *adapter,
			 struct mac_role_tbl *role)
{
	struct mac_sec_cam_info *sec_cam_info = NULL;
...
u32 mac_sta_del_key(struct mac_adapter *adapter,
		    u8 mac_id,
		    u8 key_id,
		    u8 key_type)
{
...
u32 mac_sta_add_key(struct mac_adapter *adapter,
		    struct mac_sec_cam_info *sec_cam_info,
		    u8 mac_id,
		    u8 key_id,
		    u8 key_type)
...
u32 mac_sta_search_key_idx(struct mac_adapter *adapter,
			   u8 mac_id, u8 key_id, u8 key_type)
{
	u32 sec_cam_idx = 0;
...
u32 mac_sta_hw_security_support(struct mac_adapter *adapter,
...
u32 refresh_security_cam_info(struct mac_adapter *adapter,
			      u8 mac_id)
{
