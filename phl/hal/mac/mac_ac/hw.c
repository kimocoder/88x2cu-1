		       ...
		       struct mac_ax_cctl_rty_lmt_cfg *cfg)
{
	...
	info.data_tx_cnt_lmt = cfg->data_lmt_val;
	info.rts_txcnt_lmt_sel = cfg->rts_lmt_sel;
	info.rts_txcnt_lmt = cfg->rts_lmt_val;
		    ...
		    struct mac_ax_macid_pause_cfg *cfg)
{
	...
	/* TBD, by H2C, Soar, 20191020 */
	/* TBD, H2C in val, mask format, Soar, 20200109 */
		     ...
		     enum mac_ax_hw_id hw_id, void *val)
{
	...
	case MAC_AX_HW_SET_MUEDCA_PARAM:
		...
		break;
	case MAC_AX_HW_SET_MUEDCA_TIMER:
		ret = set_hw_muedca_timer(adapter,
					  (struct mac_ax_muedca_timer *)val);
		break;
	...
	case MAC_AX_HW_SET_BW_CFG:
		...
		break;
	case MAC_AX_HW_SET_BT_BLOCK_TX:
		tx = (struct mac_ax_block_tx *)val;
		ret = cfg_block_tx(adapter, MAC_AX_BTCCA, tx->band, tx->en);
		break;
	...
	case MAC_AX_HW_SET_SCHE_PREBKF:
		...
		break;
	case MAC_AX_HW_SET_TX_NAV_BLOCK_TX:
		tx = (struct mac_ax_block_tx *)val;
		ret = cfg_block_tx(adapter, MAC_AX_TX_NAV, tx->band, tx->en);
		break;
	case MAC_AX_HW_SET_WDT_ISR_RST:
		ret = cfg_wdt_isr_rst(adapter);
		break;
	case MAC_AX_HW_SET_MAC_ERR_IMR:
		ret = mac_err_imr_ctrl(adapter, *(enum mac_ax_func_sw *)val);
		break;
...
u32 mac_get_hw_value(struct mac_adapter *adapter,
		     enum mac_hw_id hw_id, void *val)
{
	u32 ret = MACSUCCESS;
	struct mac_intf_ops *ops = adapter_to_intf_ops(adapter);
	struct mac_block_tx *tx;
	...
	case MAC_AX_HW_GET_DFLT_NAV:
		...
		break;
	case MAC_AX_HW_GET_TX_NAV_BLOCK_TX:
		tx = (struct mac_ax_block_tx *)val;
		ret = get_block_tx(adapter, MAC_AX_TX_NAV, tx->band, &tx->en);
		break;
	case MAC_AX_HW_GET_FW_CAP:
		ret = mac_get_fw_cap(adapter, (u32 *)val);
		break;
	case MAC_AX_HW_GET_RESP_ACK:
		ret = get_mac_resp_ack(adapter, (u32 *)val);
		break;
	case MAC_AX_HW_GET_HW_RTS_TH:
		ret = mac_get_hw_rts_th(adapter,
					(struct mac_ax_hw_rts_th *)val);
		break;
#endif // NEO
	default:
...
u32 cfg_mac_bw(struct mac_ax_adapter *adapter, struct mac_ax_cfg_bw *cfg)
{
	...
	if (adapter->sm.fwdl == MAC_AX_FWDL_INIT_RDY) {
		...
		if (cfg->band) {//BAND1
			ret = MAC_REG_W_OFLD(R_AX_WMAC_RFMOD_C1,
					     B_AX_WMAC_RFMOD_MSK, value8, 0);
			if (ret != MACSUCCESS) {
				PLTFM_MSG_ERR("%s: config fail\n", __func__);
				return ret;
			}
			...
			ret = MAC_REG_W_OFLD(R_AX_WMAC_RFMOD,
					     B_AX_WMAC_RFMOD_MSK, value8, 0);
			if (ret != MACSUCCESS) {
				PLTFM_MSG_ERR("%s: config fail\n", __func__);
				return ret;
