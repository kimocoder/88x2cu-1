 ...
 */
static enum rtw_hal_status
_hal_parsing_rx_wd_8822c(struct hal_info_t *hal, u8 *desc,
					struct rtw_r_meta_data *mdata)
{
	...
	if (mdata->rpkt_type == RX_8822C_DESC_PKT_T_WIFI) {
...
#endif // if 0 NEO
		mdata->bw = CHANNEL_WIDTH_160;
#if 0 // NEO
		mdata->freerun_cnt = GET_RX_AX_DESC_FREERUN_CNT_8852A(desc);

		mdata->a1_match = GET_RX_AX_DESC_A1_MATCH_8852A(desc);
#endif // if 0 NEO
		mdata->sw_dec = !!((u8)GET_RX_DESC_SWDEC_8822C(desc));
		mdata->hw_dec = !(mdata->sw_dec);
		mdata->ampdu = (u8)GET_RX_DESC_PAGGR_8822C(desc);
		mdata->ampdu_end_pkt = (u8)GET_RX_DESC_RX_EOF_8822C(desc);
		mdata->amsdu = (u8)GET_RX_DESC_AMSDU_8822C(desc);
#if 0 // NEO
...
#endif // if 0 NEO
		mdata->crc32 = (u8)GET_RX_DESC_CRC32_8822C(desc);
		mdata->icverr = (u8)GET_RX_DESC_ICV_ERR_8822C(desc);
#if 0 // NEO
		...
		mdata->with_llc = GET_RX_AX_DESC_WITH_LLC_8852A(desc);
...
#endif // if 0 NEO
			mdata->more_data = (u8)GET_RX_DESC_MD_8822C(desc);
			mdata->more_frag = (u8)GET_RX_DESC_MF_8822C(desc);
#if 0 // NEO
			mdata->pwr_bit = GET_RX_AX_DESC_PWR_8852A(desc);
#endif // if 0 NEO
			mdata->qos = (u8)GET_RX_DESC_QOS_8822C(desc);
#if 0 // NEO
			...
			mdata->seq = (u8)GET_RX_DESC_SEQ_8822C(desc);
			mdata->frag_num = (u8)GET_RX_DESC_FRAG_8822C(desc);
				...
				struct rtw_phl_rx_pkt *phl_rx)
{
	...
	case RX_8852A_DESC_PKT_T_PPDU_STATUS :
	{
		...
		if (ppdu_sts.phy_st_size != 0) {
			...
			rtw_test_setup_bp(phl_com, &bp_info, TEST_SUB_MODULE_MP);
#endif
		}
	}
	break;
	...
	case RX_8852A_DESC_PKT_T_CHANNEL_INFO :
	{
		...
		offset = phl_com->chan_info.chan_info_buffer;
		status = rtw_hal_bb_ch_info_parsing(hal, pkt->vir_addr
			, mdata->pktlen
			, offset + phl_com->chan_info.length
		...
		if (status == RTW_HAL_STATUS_BB_CH_INFO_LAST_SEG) {
			...
			/* DUMP channel info */
			debug_dump_data(phl_com->chan_info.chan_info_buffer
			, phl_com->chan_info.length, " ===RX Channel Packet=== ");
			phl_com->chan_info.length = 0;
		}
		#else
		phl_rx->type = RTW_RX_TYPE_CHANNEL_INFO;
