...
enum rtw_hal_status rtw_hal_bb_watchdog(struct hal_info_t *hal_info, u8 is_lps)
{
	...
	if (is_lps)
		halbb_watchdog_lps(hal_info->bb);
	else
		halbb_watchdog(hal_info->bb);
...
{
	halbb_ctrl_rx_cca(hal_info->bb, cca_en, phy_idx);
...
{
	halbb_init_reg(hal_info->bb, (bool)is_form_folder, folder_len,
		       folder_array, phy_idx);
	halbb_reset_bb(hal_info->bb, phy_idx);
}

void rtw_hal_init_bb_reg(struct rtw_phl_com_t *phl_com,
				struct hal_info_t *hal_info)
{
	struct rtw_para_info_t *bb_phy_reg_info = NULL;
	u8 phy_idx = 0, max_phy_num = 1;

	if (hal_info->hal_com->dbcc_en == true)
		max_phy_num = 2;

	for (phy_idx = 0; phy_idx < max_phy_num; phy_idx++) {
		bb_phy_reg_info = &phl_com->phy_sw_cap[phy_idx].bb_phy_reg_info;
		rtw_hal_bb_init_reg_by_hdr(hal_info,
				bb_phy_reg_info->para_data,
				bb_phy_reg_info->para_data_len,
				(u8)bb_phy_reg_info->para_src,
				phy_idx);
	}
}
					...
					enum channel_width bw)
{
	if(halbb_ctrl_bw_ch(hal_info->bb, pri_ch, central_ch_seg0,
				central_ch_seg1, bw, phy_idx) == false)
		return RTW_HAL_STATUS_FAILURE;
	return RTW_HAL_STATUS_SUCCESS;
}

enum rtw_hal_status rtw_hal_bb_ch_switch(void *hal, u8 central_ch, u8 cur_phy_idx)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;

	PHL_INFO("[HAL] %s\n", __FUNCTION__);
	if(central_ch > 14)
		halbb_ctrl_cck_en(hal_info->bb, false, cur_phy_idx);

	if(halbb_ctrl_ch(hal_info->bb, central_ch, cur_phy_idx) == false)
		return RTW_HAL_STATUS_FAILURE;
	if(central_ch < 36)
		halbb_ctrl_cck_en(hal_info->bb, true, cur_phy_idx);
	return RTW_HAL_STATUS_SUCCESS;
}

enum rtw_hal_status rtw_hal_bb_config_bw(void *hal, u8 pri_ch, u8 bw, u8 cur_phy_idx)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;

	PHL_INFO("[HAL] %s\n", __FUNCTION__);
	if(halbb_ctrl_bw(hal_info->bb, pri_ch, bw, cur_phy_idx) == false)
		return RTW_HAL_STATUS_FAILURE;
	else
		return RTW_HAL_STATUS_SUCCESS;
}
...
enum rtw_hal_status
rtw_hal_bb_set_plcp_pattern(struct rtw_hal_com_t *hal_com, u8 plcp_ppdu_type,
							u16 plcp_case_id, u8 plcp_phy_idx)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal_com->hal_priv;

	PHL_INFO("%s: plcp_ppdu_type = %d plcp_case_id = %d plcp_phy_idx = %d\n",
		__FUNCTION__, plcp_ppdu_type, plcp_case_id, plcp_phy_idx);

	halbb_pmac_tx_pattern(hal_info->bb,plcp_ppdu_type,plcp_case_id,plcp_phy_idx);
			...
			struct rtw_phl_com_t *phl_com, bool dbcc_en)
{
	...
	if (dbcc_en) {
		bb_phy_reg_info = &phl_com->phy_sw_cap[1].bb_phy_reg_info;
		rtw_hal_bb_init_reg_by_hdr(hal_info,
		bb_phy_reg_info->para_data,
		bb_phy_reg_info->para_data_len,
		(u8)bb_phy_reg_info->para_src,
...
enum rtw_hal_status rtw_hal_bb_ch_switch(void *hal, u8 central_ch,
					 u8 cur_phy_idx)
{
	return RTW_HAL_STATUS_SUCCESS;
}

enum rtw_hal_status rtw_hal_bb_config_bw(void *hal, u8 pri_ch, u8 bw,
					 u8 cur_phy_idx)
{
	return RTW_HAL_STATUS_SUCCESS;
}
