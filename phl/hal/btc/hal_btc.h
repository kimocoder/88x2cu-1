...
#define BTC_WRFK_MAXT 3 /* WL RFK 3*100ms */
#define BTC_BRFK_MAXT 8 /* BT RFK 8*100ms */
#define BTC_SPECPKT_MAXT 10 /* special pkt unit: 100ms */
#define BTC_RPT_HDR_SIZE 3
#define BTC_BUSY2IDLE_THRES 100 /* the wait time for WL busy-to-idle, unit:100ms */
...
enum btc_wl_gpio_debug {
	BTC_DBG_GNT_BT = 0, /* PTA signal fixed at GPIO4*/
	BTC_DBG_GNT_WL = 1,     /* PTA signal fixed at GPIO5*/
...
struct btc_chip_ops {
	...
	void (*set_wl_pri)(struct btc_t *btc, u8 mask, bool state);
...
struct btc_ops {
	...
	void (*ntfy_show_coex_info)(struct btc_t *btc,
				    void (*out)(const char *), u8 type);
	void (*ntfy_role_info)(struct btc_t *btc, u8 rid,
			      struct btc_wl_link_info *info,
			      enum role_state reason);
	void (*ntfy_control)(struct btc_t *btc, u8 type, u16 len, u8 *buf);
	void (*ntfy_radio_state)(struct btc_t *btc, bool rf_on);
	...
	void (*ntfy_fwinfo)(struct btc_t *btc, u8 *buf, u32 len, u8 cls,
			   u8 func);
	void (*ntfy_timer)(struct btc_t *btc, u32 tmr_map);
};
...
struct btc_wl_info {
	...
	u8 rsvd;
	u8 bssid[6];
	u8 port_id[PHL_MLME_MAX]; /* role to port-id mapping */
...
struct btc_dm {
	...
	u32 freerun_level: 8; /* freerun parameter level  */
	u32 rsvd: 4;
};
...
static void _ntfy_show_coex_info(struct btc_t *btc, void (*out)(const char *),
				 u8 type);
