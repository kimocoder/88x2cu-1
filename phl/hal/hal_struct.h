...
struct hal_ps_ops {
	...
	enum rtw_hal_status (*init)(struct hal_info_t *hal);
	void (*deinit)(struct hal_info_t *hal);
	u8 (*pwr_lvl_judge)(struct hal_info_t *hal_info, u8 ps_cap,
				u32 cmd_comp);
	enum rtw_hal_status (*pwr_lvl_cfg)(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
				u8 current_pwr_lvl, u8 target_pwr_lvl);
	enum rtw_hal_status (*lps_cfg)(struct hal_info_t *hal_info,
				struct rtw_hal_lps_info *lps_info);
	enum rtw_hal_status (*lps_chk_leave)(struct hal_info_t *hal_info,
				u8 macid);
};
...
struct hal_c2h_hdl {
	...
	u8 cls_min;
	u8 cls_max;
	u32 (*c2h_hdl)(void *hal, u8 cls, u8 func, u16 len, u8 *buf);
};
