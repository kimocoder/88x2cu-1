		    ...
		    u32 *val_buf)
{
	...
	BB_DBG(bb, DBG_DBG_API, "[%s][%s] LV=%d, Len=%d\n", __func__,
		...
		((pause_type == HALBB_RESUME) ? "Resume" : "Pause no_set")),
		lv, val_lehgth);
	...
	if (pause_func == F_CFO_TRK) {
		...
		ori_val[1] = (u32)(!bb->bb_dig_i.p_cur_dig_unit->dig_op_para.force_gaincode_idx_en);
			  ...
			  char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], "-h") == 0) {
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			    "{Func} {p:pause, pn:pause_no_set, r:Resume} {lv:0~3} Val[0],...,Val[5]\n");
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
