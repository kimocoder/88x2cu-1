...
enum rtw_hal_status rtw_hal_mac_ltr_sw_trigger(struct hal_info_t *hal_info, enum rtw_pcie_ltr_state state)
{
	...
	if (ret == 0) {
		...
		hal_info->hal_com->bus_cap.ltr_cur_state = ctrl;
...
void hal_mac_msg_print(void *p, s8 *fmt, ...)
{
...
#define MSG_LEN		100
...
#endif
		if (is_msg_allowed(drv_level, msg_level) == false)
			return ret;
#endif
		...
		PHL_TRACE(COMP_PHL_MAC, _PHL_ALWAYS_, "%s", str);
#endif
}
...
int rtw_hal_mac_sdio_rx(struct rtw_hal_com_t *hal, struct rtw_rx_buf *rxbuf)
{
	...
	rtw_hal_mac_sdio_parse_rx(hal, rxbuf);
...
enum rtw_hal_status rtw_hal_mac_get_pwr_state(struct hal_info_t *hal_info, u8 *pwr_state)
{
	...
	if (hal_mac_ops->get_hw_value(mac, MAC_AX_HW_GET_PWR_STATE, pwr_state) != MACSUCCESS)
		return RTW_HAL_STATUS_FAILURE;
...
bool rtw_hal_mac_reg_chk(struct rtw_hal_com_t *hal_com, u32 addr)
{
	...
	if (mac != NULL && mac->ops->check_access) {
		mac_rst = mac->ops->check_access(mac, addr);
		if (mac_rst == MACHWNOTEN) {
			rst = false;
			PHL_ERR("%s failed - addr(0x%08x) is belong to CMAC1, but CMAC1 not ready\n",
				__func__, addr);
			_os_warn_on(1);
		}
	}
...
rtw_hal_mac_ser_set_error_status(struct hal_info_t *hal_info, enum RTW_PHL_SER_RCVY_STEP err)
{
	...
	if(err == RTW_PHL_SER_L1_DISABLE_EN) {
		errtohal = MAC_AX_ERR_L1_DISABLE_EN;
	} else if (err == RTW_PHL_SER_L1_RCVY_EN) {
		errtohal = MAC_AX_ERR_L1_RCVY_EN;
	} else if (err == RTW_PHL_SER_L0_CFG_NOTIFY) {
		errtohal = MAC_AX_ERR_L0_CFG_NOTIFY;
	} else if (err == RTW_PHL_SER_L0_CFG_DIS_NOTIFY) {
		errtohal = MAC_AX_ERR_L0_CFG_DIS_NOTIFY;
	} else if (err == RTW_PHL_SER_L0_CFG_HANDSHAKE) {
		errtohal = MAC_AX_ERR_L0_CFG_HANDSHAKE;
	} else if (err == RTW_PHL_SER_L0_RCVY_EN) {
		errtohal = MAC_AX_ERR_L0_RCVY_EN;
	}
	mac_err = mac->ops->set_err_status(mac, errtohal);
...
/* #if MAC_AX_FEATURE_DBGPKG */
	...
	if(mac->ops->dump_fw_rsvd_ple(mac, &buffer) != 0) {
		PHL_ERR("%s Dump FW reserved ple area fail\n", __func__);
		return RTW_HAL_STATUS_FAILURE;
	}
...
rtw_hal_mac_cfg_disc_dec(struct hal_info_t *hal_info, u16 macid, u8 en, struct rtw_disc_det_info *cfg)
{
	...
	if (en) {
		...
		info.disconnect_en = cfg->disc_wake_en;
	...
	for (i = 0; i < MAX_WOW_CAM_NUM; ++i) {
		...
		if (en) {
			...
			if (wowcam_info->valid != 0) {
				...
				PHL_TRACE(COMP_PHL_WOW, _PHL_DEBUG_, "[%u](valid, crc) = (%u, %#x)\n", info.idx, info.valid, info.crc);
			}
		} else {
			info.idx = i;
			info.r_w = 1;
				...
				struct mac_ax_txpkt_info *txpkt_info)
{
	do {
		...
		if (txpkt_info->type == MAC_AX_PKT_H2C) {
			...
			if (txpkt_info->u.data.ndpa != 0) {
				/* Sounding related packat shall not retry */
				txpkt_info->u.data.tx_cnt_lmt_sel = 1;
				txpkt_info->u.data.tx_cnt_lmt = 1;
			} else {
				txpkt_info->u.data.tx_cnt_lmt_sel = treq->mdata.data_tx_cnt_lmt_en;
			...
			txpkt_info->u.data.a_ctrl_bsr = treq->mdata.a_ctrl_bsr;
			txpkt_info->u.data.rtt = treq->mdata.rtt_en;
			...
			txpkt_info->u.data.multiport_id = treq->mdata.hal_port;
			txpkt_info->u.data.null_0 = treq->mdata.null_0;
			txpkt_info->u.data.null_1 = treq->mdata.null_1;
			...
			/* protection */
			txpkt_info->u.data.rts_en = treq->mdata.rts_en;
			txpkt_info->u.data.cts2self = treq->mdata.cts2self;
			txpkt_info->u.data.cca_rts = treq->mdata.rts_cca_mode;
			txpkt_info->u.data.hw_rts_en = treq->mdata.hw_rts_en;
			...
			PHL_DBG("txpkt_info->u.data.macid:0x%x \n",
				 txpkt_info->u.data.macid);

			PHL_DBG("txpkt_info->pktsize:0x%x \n",
				 txpkt_info->pktsize);
			PHL_DBG("txpkt_info->u.data.tid:0x%x \n",
				 txpkt_info->u.data.tid);

			PHL_DBG("txpkt_info->u.data.data_rate:0x%x \n",
				 txpkt_info->u.data.data_rate);
			PHL_DBG("txpkt_info->u.data.agg_en:0x%x \n",
				 txpkt_info->u.data.agg_en);
			PHL_DBG("txpkt_info->u.data.bk:0x%x \n",
				 txpkt_info->u.data.bk);
			PHL_DBG("txpkt_info->u.data.max_agg_num:0x%x \n",
				 txpkt_info->u.data.max_agg_num);

			PHL_DBG("txpkt_info->u.data.wifi_seq:0x%x \n",
				 txpkt_info->u.data.wifi_seq);
			if (txpkt_info->u.data.hw_sec_en) {
				PHL_DBG("txpkt_info->u.data.sec_cam_idx=0x%x\n",
							txpkt_info->u.data.sec_cam_idx);
 ...
 *
 * Note,
 * (1) this function fix the WDBK mode and RTY_BK mode temporarily, need to move to hal_spec later
 * (2) halmac API for hal and proto type is at hal_api_mac.h
 *
 */
enum rtw_hal_status
					...
					u16 macid)
{
	...
	_os_mem_set(hal_to_drvpriv(hal_info), &dctl_info_mask, 0xFF, sizeof(struct mac_ax_dctl_info));
			...
			struct rtw_hal_lps_info *lps_info)
{
	...
	if (mac->ops->cfg_lps(mac, lps_info->macid, ax_ps_mode, &ax_lps_info)
			== MACSUCCESS)
		return RTW_HAL_STATUS_SUCCESS;
	else
		return RTW_HAL_STATUS_FAILURE;
...
{
	static u16 tx_off[2][PAUSE_RSON_MAX] = {{0}};
	...
	if (tx_pause == true) {
		...
		case PAUSE_RSON_NOR_SCAN:
			tx_off[band_idx][rson] = (u16)~RTW_TXEN_MG0;
			break;
		case PAUSE_RSON_RFK:
		case PAUSE_RSON_PSD:
		case PAUSE_RSON_DFS:
		...
		case PAUSE_RSON_RESET:
			tx_off[band_idx][rson] = (u16)RTW_TXEN_ALL;
			break;
		default:
		...
		tx_off[band_idx][rson] = 0;
	}

	for (i = 0; (i < PAUSE_RSON_MAX) && (tx_cfg != 0xFFFF); i++)
		if (tx_off[band_idx][i])
			tx_cfg |= tx_off[band_idx][i];
	/* tx_cfg is white list, but tx_off is black list */
	tx_cfg = ~tx_cfg;
...
bool rtw_hal_is_macid_pause(struct rtw_hal_com_t *hal_com, u16 macid)
{
	...
	if (ops->get_hw_value(mac, MAC_AX_HW_GET_ID_PAUSE, &cfg) != MACSUCCESS)
		PHL_ERR("%s failed\n", __func__);

	return (cfg.pause) ? true : false;
}

enum rtw_hal_status
rtw_hal_set_macid_pause(struct rtw_hal_com_t *hal_com,
			       ...
			       u8 band, enum rtw_rx_fltr_mode mode)
{
	...
	case RX_FLTR_MODE_STA_NORMAL:
		...
		if ((hal_com->chip_id == CHIP_WIFI6_8852A)
		    && (hal_com->cut_version == CHIP_CUT_A)) {
			/* 8852A A-cut issue */
		      ...
		      u32 input_num, char *output, u32 out_len)
{
	...
	if (input_num) {
		...
		if (i == hal_cmd_ary_size) {
			PHL_INFO("HAL command not found!\n");
			return;
		}
	}
	...
	switch (id) {
		...
		default:
			PHL_INFO("Do not support this command\n");
			break;
	}
}
...
s32 halmac_cmd(struct hal_info_t *hal_info, char *input, char *output, u32 out_len)
{
	...
	halmac_cmd_parser(hal_info, argv, argc, output, out_len);
...
rtw_hal_mac_set_tpu_mode(struct rtw_hal_com_t *hal_com,
			 enum rtw_tpu_op_mode op_mode_new)
{
	struct rtw_tpu_info *tpu = &hal_com->rtw_tpu_i;

	if (op_mode_new == TPU_DBG_MODE) {
		tpu->op_mode = TPU_DBG_MODE;
	} else if (op_mode_new == TPU_DBG_MODE_RELEASE) {
		tpu->op_mode = TPU_NORMAL_MODE;
	} else { /*NORMAL Mode*/
		if (tpu->op_mode == TPU_DBG_MODE) {
			tpu->normal_mode_lock_en = true;
			PHL_INFO("[1]op_mode=%d, normal_mode_lock_en=%d", tpu->op_mode, tpu->normal_mode_lock_en);
			return RTW_HAL_STATUS_FAILURE;
		}
		tpu->op_mode = TPU_NORMAL_MODE;
	}
	...
	PHL_INFO("[2]op_mode=%d, normal_mode_lock_en=%d", tpu->op_mode, tpu->normal_mode_lock_en);
	return RTW_HAL_STATUS_SUCCESS;
}
				...
				struct mac_ax_mcc_role *info)
{
	...
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_, "_hal_mac_mcc_fill_role_info(): sw_retry_count(%d), tx_null_early(%d), duration(%d)\n",
		info->sw_retry_count, info->tx_null_early, info->duration);
}
					...
					struct mac_ax_mcc_duration_info *info)
{
	...
	info->duration_x = en_info->mcc_role[0].policy.duration;
	info->duration_y = en_info->mcc_role[1].policy.duration;
	info->start_tsf_low = en_info->tsf_low;
	info->start_tsf_high = en_info->tsf_high;
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_, "_hal_mac_mcc_fill_duration_info(): info->group(%d), start_macid(%d), macid_x(%d), macid_y(%d)\n",
		info->group, info->start_macid, info->macid_x, info->macid_y);
				...
				u8 group, u8 macid, u32 tsf_high, u32 tsf_low)
{
	...
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_, ">>> rtw_hal_mac_start_mcc(): group(%d), macid(%d) Start tsf(0x%08X %08X)\n",
		group, macid, tsf_high, tsf_low);
	if (mac == NULL)
		goto exit;
	mac_status = mac->ops->start_mcc(mac, group, macid, tsf_high, tsf_low);
	if (mac_status != MACSUCCESS) {
					...
					u8 macid)
{
	...
	if (mac == NULL)
		goto exit;
	mac_status = mac->ops->stop_mcc(mac, group, macid);
	if (mac_status != MACSUCCESS) {
...
enum rtw_hal_status rtw_hal_mac_del_mcc_group(struct hal_info_t *hal, u8 group)
{
	...
	if (mac == NULL)
		goto exit;
	mac_status = mac->ops->del_mcc_group(mac, group);
	if (mac_status != MACSUCCESS) {
					...
					struct rtw_phl_mcc_en_info *en_info)
{
	...
	_hal_mac_mcc_fill_duration_info(en_info, &info);
