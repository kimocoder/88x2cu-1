				...
				void *entry)
{
	...
	if (bf_obj != NULL && bf_entry != NULL) {
		if (bf_entry->bfee != NULL) {
#ifdef RTW_WKARD_BFER_INIT
			macid_rsvd = HAL_WKARD_RSVD_MACID;
#endif
...
#ifdef RTW_WKARD_BFER_INIT
		PHL_INFO("[WARNING] 8852AE A-Cut Workaround : RTW_WKARD_BFER_INIT\n");
		macid_rsvd = HAL_WKARD_RSVD_MACID;
		for (i = 0; i < (bf_obj->max_su_bfee_nr +
		     bf_obj->max_mu_bfee_nr); i++) {
			rtw_hal_mac_ax_set_bf_entry(hal_info->mac, band,
						    (u8)macid_rsvd, i, 0, 0);
		}
#endif
		if(status != RTW_HAL_STATUS_SUCCESS)
