...
static int off_ch_return_set_ch_bw(struct scan_obj *pscan)
{
	...
	rtw_hal_set_ch_bw(phl_info->hal, wrole->hw_band,
		chandef.chan, chandef.bw, chandef.offset, false);
...
static int scan_complete(struct scan_obj *pscan)
{
	...
	if (pscan->param->wifi_role)
		rtw_hal_notify_scan_complete(phl_info->hal,
	...
	enum channel_width bw, enum chan_offset offset, u8 phy_idx)
{
	...
	rtw_hal_set_ch_bw(pscan->phl_info->hal, phy_idx,
					(u8)ch, bw, offset, false);
...
static int scan_start(struct scan_obj *pscan)
{
	...
	if (param->wifi_role)
		rtw_hal_notify_scan_start(phl_info->hal,
			param->wifi_role->hw_band);
	else
		rtw_hal_notify_scan_start(phl_info->hal, 0);
...
static int scan_idle_st_hdl(void *obj, u16 event, void *param)
{
	...
	case FSM_EV_STATE_IN:
		...
		if (chk_pending_req(pscan))
			break;
#ifdef CONFIG_PS
		/* cancel requested power service */
		phl_ps_leave_cmd_cancel((void *)pscan->phl_info,
			pscan->ps_token);
		pscan->ps_token = 0;
#endif
		break;
...
static int scan_req_pwr_st_hdl(void *obj, u16 event, void *param)
{
	...
	case FSM_EV_STATE_IN:
		...
		/* request power */
		if (pscan->ps_token == 0)
			phl_st = phl_ps_leave_cmd_async(pscan->phl_info,
					&pscan->ps_token, scan_ps_cb, pscan);
#endif
