...
static int cmd_req_pwr_st_hdl(void *obj, u16 event, void *param)
{
	...
	case FSM_EV_STATE_IN:
		...
		/* request power */
		phl_st = phl_ps_req_pwr_async(pcmd->phl_info, &pcmd->pm_token,
			(PWRCMD_COMP_IO_MAC | PWRCMD_COMP_IO_BB),
			cmd_pm_cb, pcmd, false, PHL_PS_NTFY_TYPE_CMD_IO);
	...
	case FSM_EV_CANCEL:
		phl_st = phl_ps_cancel_pwr_req(pcmd->phl_info, pcmd->pm_token);
		pcmd->pm_token = 0;
		phl_fsm_state_goto(pcmd->fsm_obj, CMD_ST_IDLE);
		break;
