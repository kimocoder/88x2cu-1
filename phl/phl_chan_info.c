...
enum rtw_phl_status
_phl_cfg_chinfo(void *phl, struct rtw_phl_stainfo_t *sta)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	if (RTW_HAL_STATUS_SUCCESS == rtw_hal_cfg_chinfo(phl_info->hal, sta))
...
phl_cmd_cfg_chinfo_hdl(struct phl_info_t *phl_info, u8 *param)
{
	...
	PHL_INFO(" %s(), sta = %p !\n", __func__, sta);
	return _phl_cfg_chinfo(phl_info, sta);
}

enum rtw_phl_status
_phl_cmd_cfg_chinfo(void *phl, struct rtw_phl_stainfo_t *sta,
		    enum phl_cmd_type cmd_type, u32 cmd_timeout)
{
	enum rtw_phl_status sts = RTW_PHL_STATUS_FAILURE;
	if (cmd_type == PHL_CMD_DIRECTLY) {
		return _phl_cfg_chinfo(phl, sta);
	}
...
#ifdef CONFIG_CMD_DISP
	return _phl_cmd_cfg_chinfo(phl, sta, cmd_type, cmd_timeout);
#else
	return _phl_cfg_chinfo(phl_info, sta);
#endif
}
