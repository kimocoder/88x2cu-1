...
{
	u8 gi = (guard_int & BIT(RTW_GILTF_1XHE08)) ? 2 : 1;
	u8 mcs_idx = he_mcs_rate - MGN_HE1SS_MCS0;

	return HE_MCS_DATA_RATE[bw][gi][mcs_idx];
}

static u8 rtw_he_get_highest_rate(struct rtw_phl_stainfo_t *phl_sta)
{
	u8 i, j;
	u8 bit_map;
	u8 he_mcs_rate = 0;
	...
	for (i = 0; i < 2; i++) {
		if (phl_sta->asoc_cap.he_rx_mcs[i] != 0xff) {
			/* max to 4SS, each SS contains 2 bit */
			for (j = 0; j < 8; j += 2) {
				bit_map = (phl_sta->asoc_cap.he_rx_mcs[i] >> j) & 3;
...
static void HE_supp_mcs_handler(_adapter *padapter, struct rtw_phl_stainfo_t *phl_sta, u8 *ele_start, u8 supp_mcs_len)
{
	...
	rtw_he_set_asoc_cap_supp_mcs(padapter, phl_sta, ele_start, supp_mcs_len);
	phepriv->he_highest_rate = rtw_he_get_highest_rate(phl_sta);
}
