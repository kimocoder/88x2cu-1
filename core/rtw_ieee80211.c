...
/* Update same length IE content */
u8 *rtw_update_ie(u8 *pbuf, u8 eid, u8 content_len, const u8 *content)
{
	if ((*pbuf != eid) || (*(pbuf + 1) != content_len))
		return _FAIL;

	_rtw_memcpy((void *)(pbuf + 2), (void *)content, content_len);

	return pbuf + content_len + 2;
}
