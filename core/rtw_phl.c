...
struct rtw_phl_mcc_ops rtw_mcc_ops = {
	.priv = NULL,
	.mcc_update_noa = rtw_mcc_update_noa,
};
#endif
		      ...
		      u8 offset, u8 do_rfk)
{
	...
	status = rtw_phl_set_ch_bw(a->phl_role,
				   ch, bw, offset, do_rfk);
...
u8 rtw_hw_iface_type_change(_adapter *adapter, u8 iface_type)
{
	...
	status = rtw_phl_wifi_role_change(phl, wrole, WR_CHG_TYPE,
		(void*)&rtype);
	if (status != RTW_PHL_STATUS_SUCCESS) {
...
static int rtw_hw_chk_sec_mode(struct _ADAPTER *a, struct sta_info *sta)
{
	...
	if (sec_mode != sta->phl_sta->sec_mode) {
		...
		status = rtw_phl_change_stainfo(phl, sta->phl_sta, STA_CHG_SEC_MODE, &sec_mode);
		   ...
		   u8 keyid, enum security_type keyalgo, u8 keytype, u8 *key)
{
	...
	if (rtw_hw_chk_sec_mode(a, sta) == _FAIL)
		return -1;
	...
	rtw_phl_del_key(phl, sta->phl_sta, &crypt, 1, NULL);
	status = rtw_phl_add_key(phl, sta->phl_sta, &crypt, key, 1, NULL);
	if (status != RTW_PHL_STATUS_SUCCESS)
		return -1;
		   ...
		   u8 keyid, u8 keytype)
{
	...
	status = rtw_phl_del_key(phl, sta->phl_sta, &crypt, 1, NULL);
	if (status != RTW_PHL_STATUS_SUCCESS)
		return -1;
...
int rtw_hw_connect_abort(struct _ADAPTER *a, struct sta_info *sta)
{
	...
	rtw_hw_del_all_key(a, sta);

	status = rtw_phl_update_media_status(phl, sta->phl_sta, NULL, false);
	if (status != RTW_PHL_STATUS_SUCCESS)
		return -1;
...
int rtw_hw_set_edca(struct _ADAPTER *a, u8 ac, u32 param)
{
	...
	edca.ac = ac;
	edca.param = param;
	status = rtw_phl_wifi_role_change(phl, a->phl_role, WR_CHG_EDCA_PARAM,
					  &edca);
	if (status != RTW_PHL_STATUS_SUCCESS) {
...
int rtw_hw_connected(struct _ADAPTER *a, struct sta_info *sta)
{
	...
	status = rtw_phl_update_media_status(phl, sta->phl_sta,
...
int rtw_hw_connected_apmode(struct _ADAPTER *a, struct sta_info *sta)
{
	...
	if (RTW_PHL_STATUS_SUCCESS != rtw_phl_update_media_status(
