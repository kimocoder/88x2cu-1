...
void rtw_vht_ies_attach(_adapter *padapter, WLAN_BSSID_EX *pnetwork)
{
	...
	pmlmepriv->ext_capab_ie_len = EXT_CAP_IE_LEN;
	rtw_set_ie(pnetwork->IEs + pnetwork->IELength, EID_EXTCapability,
		EXT_CAP_IE_LEN, pmlmepriv->ext_capab_ie_data, &len);
	pnetwork->IELength += len;
...
void rtw_set_vht_ext_cap(_adapter *padapter, WLAN_BSSID_EX *pnetwork)
{
	...
	pbuf = rtw_get_ie(ies, EID_EXTCapability, &ie_len, ies_len);
	if (pbuf == NULL) {
		/* VHT Operation mode notifiy bit in extended capability IE */
		rtw_set_ie(ies + ies_len, EID_EXTCapability, EXT_CAP_IE_LEN,
			pmlmepriv->ext_capab_ie_data, &frlen);
		pnetwork->IELength += frlen;
	} else {
		rtw_update_ie(pbuf, EID_EXTCapability, EXT_CAP_IE_LEN, pmlmepriv->ext_capab_ie_data);
	}
}
