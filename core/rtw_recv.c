	...
	if ((psta->state & WIFI_SLEEP_STATE) && (rtw_tim_map_is_set(adapter, pstapriv->sta_dz_bitmap, psta->phl_sta->aid))) {
		...
		if ((rtw_end_of_queue_search(xmitframe_phead, xmitframe_plist)) == _FALSE) {
			...
			if (psta->sleepq_len == 0) {
				...
				update_beacon(adapter, _TIM_IE_, NULL, _TRUE, 0);
			}
			/* _rtw_spinunlock_bh(&psta->sleep_q.lock); */
...
static void rx_process_rssi(_adapter *padapter, union recv_frame *prframe)
{
	...
	/* if(pRfd->Status.bPacketToSelf || pRfd->Status.bPacketBeacon) */
	{
		...
		if (padapter->recvinfo.is_signal_dbg) {
			adapter_to_dvobj(padapter)->recvpriv.signal_strength = padapter->recvinfo.signal_strength_dbg;
			adapter_to_dvobj(padapter)->recvpriv.rssi = (s8)rtw_phl_rssi_to_dbm(padapter->recvinfo.signal_strength_dbg);
		} else {
...
s32 rtw_core_rx_data_pre_process(_adapter *adapter, union recv_frame *prframe)
{
	...
	prframe = decryptor(adapter, prframe);
	if (prframe == NULL)
		return CORE_RX_DROP;
	...
	prframe = portctrl(adapter, prframe);
	if (prframe == NULL)
		return CORE_RX_DROP;
