		...
		u32 out_num, struct rtw_ieee80211_channel *in, u32 in_num, bool no_sparse)
{
	...
	for (i = 0; i < in_num; i++) {
		...
		if (set_idx >= 0) {
			...
			if (rfctl->channel_set[set_idx].ScanType == RTW_PHL_SCAN_PASSIVE
	...
	if (j == 0) {
		for (i = 0; i < rfctl->max_chan_nums; i++) {
			...
			if (rtw_mlme_band_check(padapter, chan) == _TRUE) {
				...
				if (rfctl->channel_set[i].ScanType == RTW_PHL_SCAN_PASSIVE
...
#if 1 /*#ifndef CONFIG_PHL_ARCH*/ /*NEO need to change back*/
			...
			enum rtw_phl_scan_type *type)
{
	...
	if (pwdinfo->rx_invitereq_info.scan_op_ch_only || pwdinfo->p2p_info.scan_op_ch_only) {
		...
		*/
		...
		if (ch_set_idx >= 0)
			stype = rfctl->channel_set[ch_set_idx].ScanType;
		else
			stype = RTW_PHL_SCAN_ACTIVE;
	} else
#endif /* CONFIG_P2P */
...
operation_by_state:
	...
	case SCAN_COMPLETE:
		...
		report_surveydone_event(padapter, ss->acs, RTW_CMDF_WAIT_ACK);
#ifdef CONFIG_RTW_ACS
		if (IS_ACS_ENABLE(padapter))
...
#ifdef RTW_WKARD_ENLARGE_SCAN_TO
	RTW_INFO("%s , scan_timeout_ms = org- %d (ms) ,chg- %d (ms)\n",
		__func__, ss->scan_timeout_ms, SCANNING_TIMEOUT_FIX);
	ss->scan_timeout_ms = SCANNING_TIMEOUT_FIX;
#endif
	...
	struct rtw_phl_scan_param *phl_param, struct sitesurvey_parm *param)
{
	...
	for (i = 0; i < param->ch_num; i++) {
		...
		if (param->ch[i].flags & RTW_IEEE80211_CHAN_PASSIVE_SCAN) {
			...
			if (param->scan_type == RTW_SCAN_NORMAL)
