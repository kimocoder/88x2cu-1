...
chbw_decision:
	...
	parm->ch_to_set = ch_to_set;
	parm->offset_to_set = offset_to_set;
	parm->bw_to_set = bw_to_set;
	parm->do_rfk = do_rfk;
...
int rtw_check_beacon_data(_adapter *padapter, u8 *pbuf,  int len)
{
	...
	if(padapter->registrypriv.ht_enable &&
		...
		p = rtw_get_ie(ie + _BEACON_IE_OFFSET_, EID_EXTCapability, 
			&ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_));
		...
		if (p == NULL) {
			...
			rtw_update_ie(p, EID_EXTCapability, EXT_CAP_IE_LEN, pmlmepriv->ext_capab_ie_data);
		}
	}
#endif /* CONFIG_80211N_HT */
