...
chbw_decision:
	...
	parm->offset_to_set = chdef_to_set.bw;
	parm->bw_to_set = chdef_to_set.offset;
	parm->do_rfk = do_rfk;
...
static u8 rtw_ap_ch_specific_chk(_adapter *adapter, u8 ch, u8 *bw, u8 *offset, const char *caller)
{
	...
	if (chset[ch_idx].ScanType == RTW_PHL_SCAN_PASSIVE) {
	...
	while (!rtw_chset_is_chbw_valid(chset, ch, *bw, *offset, 0, 0)
		|| (rtw_hal_dfs_domain_unknown(dvobj) && rtw_chset_is_dfs_chbw(chset, ch, *bw, *offset))
	) {
	...
	if (rtw_hal_dfs_domain_unknown(dvobj) && rtw_chset_is_dfs_chbw(chset, ch, *bw, *offset)) {
...
#if defined(CONFIG_DFS_MASTER)
	if (!rtw_hal_dfs_domain_unknown(dvobj)) {
...
int rtw_ap_addr_resolve(_adapter *adapter, u16 os_qid, struct xmit_frame *xframe,  struct sk_buff *pkt, _list *b2u_list)
{
	...
	if (is_da_mcast) {
		...
		if (rtw_ap_src_b2u_policy_chk(adapter->b2u_flags_ap_src, etherhdr.h_dest)
			...
			bmc_need = rtw_ap_data_bmc_to_uc(adapter , etherhdr.h_dest,
...
static enum phl_mdl_ret_code _ap_start_req_ev_hdlr(void *dispr, void *priv, struct phl_msg* msg)
{
	...
	switch(MSG_EVT_ID_FIELD(msg->msg_id)) {
		case MSG_EVT_AP_START_PREPARE:
		{
			...
			if(status != RTW_PHL_STATUS_SUCCESS)
				RTW_ERR("%s :: [START_PREPARE] rtw_phl_send_msg_to_dispr failed\n", __func__);
			break;
		}
		...
		case MSG_EVT_AP_START:
		{
			...
			_ap_start_end_notify(padapter, true, false);
			break;
		}
...
static enum phl_mdl_ret_code _ap_stop_req_ev_hdlr(void *dispr, void *priv, struct phl_msg* msg)
{
	...
	switch(MSG_EVT_ID_FIELD(msg->msg_id)) {
		case MSG_EVT_AP_STOP_PREPARE:
		{
			...
			_ap_stop_ap_stop_notify(padapter);
			break;
		}
