 ...
 *
 *****************************************************************************/
#define _RTL8822CU_HALMAC_C_

#include <drv_types.h>		/* struct dvobj_priv and etc. */
#include "../../hal_halmac.h"
#include "../rtl8822c.h"	/* rtl8822c_cal_txdesc_chksum() and etc. */
...
static u8 usb_write_data_not_xmitframe(void *d, u8 *pBuf, u32 size, u8 qsel)
{
	...
	if (qsel == HALMAC_TXDESC_QSEL_BEACON) {
		...
		if (add_pkt_offset == 1) {
			SET_TX_DESC_OFFSET_8822C(buf, desclen + PACKET_OFFSET_SZ);
			SET_TX_DESC_PKT_OFFSET_8822C(buf, 1);
		} else
	...
	rtl8822c_cal_txdesc_chksum(padapter, buf);
	...
	if (_SUCCESS == ret) {
		...
		RTW_ERR("%s , rtl8822cu_simple_write_port fail\n", __func__);
		rtw_mfree(buf , len);
		ret = _FALSE;
	}
...
int rtl8822cu_halmac_init_adapter(PADAPTER padapter)
{
	struct dvobj_priv *d;
